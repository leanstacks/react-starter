...
const Toast = ({ className, dismiss, testId = 'toast', toast }: ToastProps) => {
  ...
  /**
   * Effect hook to handle auto dismissing the toast. If the toast is set to auto dismiss, an interval is
   * created to check if the toast should be dismissed based on its creation time and the configured auto
   * dismiss duration. If the toast is not set to auto dismiss, no interval is created to avoid memory leaks.
   */
  useEffect(() => {
    // auto dismiss toasts need an interval to check if they should be dismissed
    ...
    // manually dismissed toasts do not need an interval, so return undefined to avoid a memory leak
    return undefined;
  }, [toast]);
